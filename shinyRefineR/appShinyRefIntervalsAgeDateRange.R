  ...
  tabPanel(
    ...
    sidebarLayout(sidebarPanel(
      ...
      dateRangeInput("dateRange", "Select Date Range:", 
                     start = min(Combined_Data_KC$Datum), 
                     end = max(Combined_Data_KC$Datum)),
...
server <- function(input, output, session) {
  ...
  dateRange <- reactive({
    Combined_Data_KC |> 
      filter(Bezeichnung == input$analyte &
               Datum >= input$dateRange[1] &
               Datum <= input$dateRange[2])
  })|> bindCache(input$analyte, input$dateRange, cache = "session")
  ...
  refIntervalBoth <- reactive({
    ...
    filteredData <- Combined_Data_KC |> 
      filter(Combined_Data_KC$Bezeichnung == input$analyte &
               Datum >= input$dateRange[1] &
               Datum <= input$dateRange[2])
    ...
    findRI(Data = values)
  })|> bindCache(Combined_Data_KC, input$analyte, input$dateRange,cache = "session")
